...
```sql
Create	tablespace	hyl_space1datafile '/home/oracle/app/oracle/oradata/orcl/hylpdb/hyl_space1.dbf' size 150M autoextend on next 50m maxsize unlimited;
 
create tablespace hyl_space2 DATAFILE
'/home/oracle/app/oracle/oradata/orcl/hylpdb/hyl_space2.bdf' SIZE 50M AUTOEXTEND ON NEXT 5M MAXSIZE 100M;
 
create tablespace hyl_space3 DATAFILE
'/home/oracle/app/oracle/oradata/orcl/hylpdb/hyl_space3.bdf' SIZE 50M AUTOEXTEND ON NEXT 5M MAXSIZE 100M;
 
create tablespace hyl_space4 DATAFILE
...
1. 分别创建两个用户hyl_user1和hyl_user2,并允许他们是使用hyl_space1表空间

```sql
create user hyl_user1 IDENTIFIED by 123;
create user hyl_user2 IDENTIFIED by 123;
alter user hyl_user1 quota unlimited on hyl_space1;
...
```sql
create role hyl_role1;
create role hyl_role2;
grant select any table to hyl_role1;
grant select any table to hyl_role2;
...
declare
  ...
  price float;
begin
  insert into book(book_no,book_name,book_class,book_author,book_num,book_price,book_out) values ('1','java从入门到精通','计算机','hyl',100,12.5,'人民出版社');
  insert into book(book_no,book_name,book_class,book_author,book_num,book_price,book_out) values ('2','Python从入门到精通','计算机','hyl',200,22.5,'人民出版社');
  insert into book(book_no,book_name,book_class,book_author,book_num,book_price,book_out) values ('3','哈利波特','科幻','hyl',400,50.5,'国外出版社');
  insert into book(book_no,book_name,book_class,book_author,book_num,book_price,book_out) values ('4','C/C++从入门到精通','计算机','hyl',120,12.5,'人民出版社');
